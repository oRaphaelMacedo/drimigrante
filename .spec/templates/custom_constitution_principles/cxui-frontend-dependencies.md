...
Before running `/spec.clarify` or `/spec.plan`, the AI MUST:
- Collect and validate absolute local paths for all five repositories.
- Read one architecture file from each repository (prefer `.spec/memory/architecture.md`, fallback `architecture.md`).
