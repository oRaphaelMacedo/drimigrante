...
**Tests**: Tests are REQUIRED. If TDD enabled (tdd=true), tests appear BEFORE implementation in each user story phase. If TDD disabled (tdd=false), tests appear AFTER implementation in the Polish phase.
...
### Tests for User Story 1 (TDD=true: here BEFORE impl | TDD=false: in Polish phase)

> **NOTE (TDD mode only)**: Write these tests FIRST, ensure they FAIL before implementation
...
### Tests for User Story 2 (TDD=true: here BEFORE impl | TDD=false: in Polish phase)
...
### Tests for User Story 3 (TDD=true: here BEFORE impl | TDD=false: in Polish phase)
...
**Purpose**: Improvements that affect multiple user stories, and tests (if TDD=false)

### Tests (if TDD disabled - tests after implementation)

> **NOTE**: When TDD is disabled, all test tasks are placed here after implementation is complete.

- [ ] TXXX [P] [ASYNC] Unit tests for User Story 1 in tests/unit/
- [ ] TXXX [P] [ASYNC] Unit tests for User Story 2 in tests/unit/
- [ ] TXXX [P] [ASYNC] Unit tests for User Story 3 in tests/unit/
- [ ] TXXX [P] [ASYNC] Integration tests in tests/integration/
- [ ] TXXX [P] [ASYNC] Contract tests in tests/contract/

### Other Polish Tasks

- [ ] TXXX [P] [ASYNC] Documentation updates in docs/
- [ ] TXXX [ASYNC] Code cleanup and refactoring
- [ ] TXXX [SYNC] Performance optimization across all stories
- [ ] TXXX [SYNC] Security hardening
- [ ] TXXX [ASYNC] Run quickstart.md validation
...
- **If TDD enabled**: Tests MUST be written and FAIL before implementation (tests in story phase)
- **If TDD disabled**: Implementation first, tests in Polish phase after all stories complete
...
```bash
# Launch all tests for User Story 1 together (if TDD enabled):
