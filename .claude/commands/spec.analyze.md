...
**Architecture Cross-Validation** (NEW): When architecture artifacts exist (`.spec/memory/architecture.md` or `specs/{feature}/architecture.md`), validate spec and plan alignment with system and feature-level architecture constraints.
...
Run `.spec/scripts/bash/check-prerequisites.sh --json --include-tasks` once from repo root and parse JSON for FEATURE_DIR and AVAILABLE_DOCS. Derive absolute paths:
...
- Load `.spec/memory/constitution.md` for principle validation (both modes)

**From architecture (if exists):**

- Load `.spec/memory/architecture.md` for system-level architecture context (includes ADRs in Section 6)
