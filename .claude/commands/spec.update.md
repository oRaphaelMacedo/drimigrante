...
2. **Verify Script Exists**
   - Check that the update script is present in `.spec/scripts/`
...
1. **Run Update Script**
   ...
   **macOS/Linux:**
   ```bash
   bash .spec/scripts/bash/update-spec-kit.sh
   ```

   **Native PowerShell:**
   ```powershell
   .\.spec\scripts\powershell\update-spec-kit.ps1
   ```
   
   **Windows from bash/Git Bash:**
   ```bash
   powershell.exe -NoLogo -NoProfile -NonInteractive -File ./.spec/scripts/powershell/update-spec-kit.ps1
   ```
...
1. **Updated Successfully**
   ...
   📝 Files updated (15):
      ...
      • .spec/scripts/bash/update-spec-kit.sh
      • .codex/skills/spec.specify/SKILL.md
      • ... (and more)

   ✨ Files added (2):
      • .spec/templates/new-template.md
      • .codex/skills/spec.checklist/SKILL.md
...
3. **Error Occurred**
   ...
   1. Visiting https://github.com/CheckmarxDev/internal-cx-agents/tree/main/spec-kit
   2. Downloading the latest files
   3. Replacing the files in your .spec/ directory
   ```
...
|-----------|----------|-------------|
| Scripts | `.spec/scripts/bash/` | Bash automation scripts |
| Scripts | `.spec/scripts/powershell/` | PowerShell automation scripts |
| Templates | `.spec/templates/` | Specification templates |
...
| Config | `.spec/config.json` | Version updated, new keys added |
...
- **Local version** is read from `.spec/config.json`
