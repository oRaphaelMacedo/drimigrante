---
description: Generate feature checklist from requirements
---
...
1. **Setup**: Run `.spec/scripts/bash/check-prerequisites.sh --json` from repo root and parse JSON for FEATURE_DIR and AVAILABLE_DOCS.
...
4. **Load feature context**: Read from FEATURE_DIR:
     ...
     - .spec/config/config.json: Current mode and enabled options (under `workflow` and `options` sections)
