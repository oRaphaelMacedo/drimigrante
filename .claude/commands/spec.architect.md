...
- **init**: Creates `.spec/memory/architecture.md` from template when no architecture exists
...
The text the user typed after `/spec.architect` in the triggering message **is** the architecture action (init/map/update/review) and context. Assume you always have it available in this conversation even if `$ARGUMENTS` appears literally below.

### If No Arguments Provided

When user runs `/spec.architect` without specifying an action, **always show help first**:
...
1. **Display Available Actions**:
   ...
   Usage: /spec.architect <action> [description]
   ```

2. **Auto-Detect and Recommend**:
   - Check if `.spec/memory/architecture.md` exists
...
2. **Load Current State**:
   - Check if `.spec/memory/architecture.md` exists
   - Load `.spec/memory/constitution.md` for alignment validation
   - Check for team directives if available

3. **Execute Appropriate Phase**: Run Phases 0-5 based on action type

4. **Generate or Update Architecture**: Write/update `.spec/memory/architecture.md`
...
The diagram format is configured in repo config at `.spec/config.json`:
...
1. Edit `.spec/config.json`
2. Change `diagram_format` to either `"mermaid"` or `"ascii"`
3. Run `/spec.architect update` to regenerate diagrams
...
- **Strategic Context** for all `/spec.specify` and `/spec.plan` commands
...
- The `.spec/memory/architecture.md` file provides **global context** inherited by all feature specs
...
- `/spec.plan` reads architecture for **Global Constraints** validation
...
1. **Identify Stakeholders**:
   - Read `.spec/memory/constitution.md` if it exists
...
- Use the template structure from `{REPO_ROOT}/.spec/templates/architecture-template.md`
...
1. **Diff Analysis**:
   - Compare current `.spec/memory/architecture.md` with recent code/spec changes
...
1. **Constitution Alignment**:
   - Load `.spec/memory/constitution.md`
...
- **Primary Output**: `.spec/memory/architecture.md`
...
- "✅ Architecture generated: .spec/memory/architecture.md"

## Key Rules

### Constitution Compliance

- Architecture MUST align with `.spec/memory/constitution.md` principles
...
### After `/spec.architect init`

Recommended next steps:

1. Review generated architecture with stakeholders
2. Refine sections that need more detail
3. Run `/spec.architect review` to validate
4. Switch to normal workflow: `/spec.specify` → `/spec.plan` → `/spec.tasks` → `/spec.implement`

### After `/spec.architect map`
...
4. Consider running `/spec.architect review`
