...
**Note**: Architecture is enabled by default in both modes. Use `--no-architecture` to disable feature-level `specs/{feature}/architecture.md` during `/spec.plan`.
...
1. **Analyze the feature description**:
   - Understand the core goal and scope.

2. **Generate a concise short name** (2-4 words) for the feature name:
...
3. **Run create-new-feature exactly once** with mode/options and the original description:
   - Bash example:
     ```bash
     .spec/scripts/bash/create-new-feature.sh --json --short-name "user-auth" --mode spec --tdd true --contracts true --data-models true --risk-tests true "Add user authentication"
     ```
   - Native PowerShell:
     ```powershell
     .\.spec\scripts\powershell\create-new-feature.ps1 -Json -ShortName "user-auth" -Mode spec -Tdd -Contracts -DataModels -RiskTests "Add user authentication"
     ```
   - Windows from bash/Git Bash:
     ```bash
     powershell.exe -NoLogo -NoProfile -NonInteractive -File ./.spec/scripts/powershell/create-new-feature.ps1 -Json -ShortName "user-auth" -Mode spec -Tdd -Contracts -DataModels -RiskTests "Add user authentication"
     ```

   **IMPORTANT**:
   - The generated branch/spec directory format is `feature/<short-name>` (example: `feature/add-login`).
