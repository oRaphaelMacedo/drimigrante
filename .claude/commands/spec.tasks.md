...
1. **Setup**: Run `.spec/scripts/bash/check-prerequisites.sh --json` from repo root and parse FEATURE_DIR and AVAILABLE_DOCS list. All paths must be absolute. For single quotes in args like "I'm Groot", use escape syntax: e.g 'I'\''m Groot' (or double-quote if possible: "I'm Groot").
...
2. **Initialize Dual Execution Loop** ⚠️ **MANDATORY - DO NOT SKIP**:
   ...
   **You MUST run this command before proceeding:**
   - Bash: `.spec/scripts/bash/tasks-meta-utils.sh init "$FEATURE_DIR"`
   - Native PowerShell: `.\.spec\scripts\powershell\tasks-meta-utils.ps1 init "$FEATURE_DIR"`
   - Windows from bash/Git Bash: `powershell.exe -NoLogo -NoProfile -NonInteractive -File ./.spec/scripts/powershell/tasks-meta-utils.ps1 init "$FEATURE_DIR"`
...
4. **Execute task generation workflow** (follow the template structure):
    ...
    - **If risk-based testing is enabled in mode configuration**:
      - Run `.spec/scripts/bash/generate-risk-tests.sh` with combined SPEC_RISKS and PLAN_RISKS from .spec/scripts/bash/check-prerequisites.sh --json output
    ...
    - **Generate tasks ORGANIZED BY USER STORY**:
      ...
      - For each user story (in priority order P1, P2, P3...):
        ...
        - **For each task**: Run the classify command to determine execution mode and update tasks_meta.json:
          - Bash: `.spec/scripts/bash/tasks-meta-utils.sh classify "$task_description" "$task_files"` then `.spec/scripts/bash/tasks-meta-utils.sh add-task "$FEATURE_DIR/tasks_meta.json" "$task_id" "$task_description" "$task_files" "$execution_mode"`
          - Native PowerShell: `.\.spec\scripts\powershell\tasks-meta-utils.ps1 classify "$task_description" "$task_files"` then `.\.spec\scripts\powershell\tasks-meta-utils.ps1 add-task "$FEATURE_DIR/tasks_meta.json" "$task_id" "$task_description" "$task_files" "$execution_mode"`
          - Windows from bash/Git Bash: `powershell.exe -NoLogo -NoProfile -NonInteractive -File ./.spec/scripts/powershell/tasks-meta-utils.ps1 classify "$task_description" "$task_files"` then `powershell.exe -NoLogo -NoProfile -NonInteractive -File ./.spec/scripts/powershell/tasks-meta-utils.ps1 add-task "$FEATURE_DIR/tasks_meta.json" "$task_id" "$task_description" "$task_files" "$execution_mode"`
...
5. **Generate tasks.md**: Use `{REPO_ROOT}/.spec/templates/tasks-template.md` as structure, fill with:
