...
0. **Custom Constitution Principles Gate**
   - Execute once from repo root:
     - Bash: `.spec/scripts/bash/check-custom-constitution-principles.sh --json`
     - Native PowerShell: `.\.spec\scripts\powershell\check-custom-constitution-principles.ps1 -Json`
     - Windows from bash/Git Bash: `powershell.exe -NoLogo -NoProfile -NonInteractive -File ./.spec/scripts/powershell/check-custom-constitution-principles.ps1 -Json`
...
1. **Environment Initialization**
   - Execute: `.spec/scripts/bash/setup-plan.sh --json` from repository root
...
2. **Context Acquisition**
   ...
   - **Constitutional Loading:** Read `.spec/memory/constitution.md` for governance rules
   - **Template Loading:** Load appropriate template based on mode
     - **Build Mode**: Use `{REPO_ROOT}/.spec/templates/plan-template-build.md` (lightweight structure)
     - **Spec Mode**: Use `{REPO_ROOT}/.spec/templates/plan-template.md` (full research-driven structure)
...
3. **Agent context update**:
    - macOS/Linux: run `.spec/scripts/bash/update-agent-context.sh`
    - Native PowerShell: run `.\.spec\scripts\powershell\update-agent-context.ps1`
    - Windows from bash/Git Bash: run `powershell.exe -NoLogo -NoProfile -NonInteractive -File ./.spec/scripts/powershell/update-agent-context.ps1`
...
1. **Load System Architecture**:
   - Read `.spec/memory/architecture.md` for system-level architecture context (includes ADRs in Section 6)
...
3. **Document Feature ADRs**:
   ...
   - Each ADR should reference system ADRs from `.spec/memory/architecture.md`: "Aligns with ADR-XXX"
   - Flag any conflicts: "VIOLATION: Conflicts with ADR-XXX"

4. **Generate Feature Architecture Description**:
   - Use `.spec/templates/architecture-template.md` as base
...
- Feature ADRs are auto-validated against system ADRs in `.spec/memory/architecture.md` (Section 6)
...
|-------|----------|-------------------|--------------|
| **System** | Main branch | `.spec/memory/architecture.md` (ADRs in Section 6) | `/spec.architect.*` commands |
| **Feature** | Feature branch | `specs/{feature}/architecture.md` | `/spec.plan --architecture` |
