...
- Parse JSON from `.spec/scripts/bash/check-prerequisites.sh --json --paths-only` to get paths including `CONSTITUTION`, `ARCHITECTURE`, and existence flags
...
1. Run `.spec/scripts/bash/check-prerequisites.sh --json --paths-only` from repo root **once** (combined `--json --paths-only` mode / `-Json -PathsOnly`). Parse minimal JSON payload fields:
   - `FEATURE_DIR`
   - `FEATURE_SPEC`
